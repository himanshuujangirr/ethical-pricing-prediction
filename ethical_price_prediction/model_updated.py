...
def assess_price(item_category, item_name, city):
    ...
    try:
        ...
        # Plot 3: Monthly Price Trend (FIXED)
        try:
            if "month" not in df.columns:
                df["month"] = np.random.randint(1, 13, size=len(df))  # Dummy months for display

            monthly_df = df[(df["item_name"] == item_name) & (df["city"] == city)]

            if not monthly_df.empty:
                monthwise = monthly_df.groupby("month")["base_price"].mean().reset_index()
                if not monthwise.empty:
                    plt.figure(figsize=(8, 5))
                    sns.lineplot(data=monthwise, x="month", y="base_price", marker="o")
                    plt.title(f"Monthly Price Trend for {item_name} in {city}")
                    plt.xlabel("Month")
            ...
            print("Monthly trend graph error:", trend_error)
